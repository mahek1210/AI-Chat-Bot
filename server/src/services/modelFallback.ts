...
export const MODEL_FALLBACKS = {
  gemini: {
    ...
    'google/gemini-1.5-flash': ['google/gemini-1.5-pro', 'gemini-1.0-pro'],
...
export class ModelFallbackHandler {
  ...
  static suggestAlternatives(model: string, provider: string, error: any): string[] {
    ...
    if (this.isModelNotFoundError(error)) {
      if (provider === 'gemini') {
        suggestions.push('gemini-1.5-pro', 'gemini-1.0-pro');
      } else if (provider === 'openai') {
        suggestions.push('gpt-4o-mini', 'gpt-3.5-turbo');
  ...
  static createErrorResponse(
    ...
    return {
      ...
      suggestions: suggestions.length > 0 
        ? suggestions 
        : ['gpt-3.5-turbo', 'gemini-1.0-pro', 'claude-3-haiku-20240307'],
    };
  }
}
