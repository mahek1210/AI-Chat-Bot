...
export class AIRouter {
   ...
   * @returns A promise resolving to the LLM response with fallback information
   */
  public async routeRequest(request: LLMRequest): Promise<LLMResponse & { fallbackInfo?: any }> {
    if (!request.model) {
    ...
    try {
      ...
      if (provider === 'openai' && this.isQuotaError(error)) {
        ...
        const fallbackResponse = await this.fallbackToOpenRouter(request);
        ...
        return {
          ...
          fallbackInfo: {
            provider: 'openrouter',
            model: this.mapToOpenRouterModel(originalModel),
            fallbackUsed: true,
